...
- **全流程中文交流**：所有文档、注释、提示词、报告、答辩材料以及 AI 与用户的交互全程使用中文（代码关键字、技术专有名词、第三方库名除外）。
- **修改与优化需人工确认**：每次修改或优化建议在实施前必须经用户明确确认，不擅自改动任何代码。
- **测试不自动运行**：未经用户同意不自动执行任何测试，仅向用户提供测试指令和脚本，由用户自行决定是否运行。
- **脚本需讲解与中文注释**：每个脚本必须附带简单讲解（用途、输入、输出、运行方式），脚本中关键步骤必须有中文注释。
...
- 始终优先稳定、可运行、可展示。
- 每次修改或优化前必须请求用户确认，不经授权不执行。
- 不自动运行测试，仅提供带有中文注释和讲解的测试指令与脚本。
- 每个交付脚本附带用途说明、输入输出描述和运行方式。
